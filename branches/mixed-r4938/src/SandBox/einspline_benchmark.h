...
namespace qmcplusplus
{
  ...
  struct einspline3d_benchmark
  {
    ...
    {
      create_plan(nx, ny, nz, num_splines);
    }

    void create_plan(int nx, int ny, int nz, int num_splines)
    {
      TinyVector<int, 3> ng(nx, ny, nz);
      einspliner.create_plan(ng,num_splines);
      Array<value_type, 3> data(nx, ny, nz);
      for (int i = 0; i < num_splines; ++i)
      {
        for (int j = 0; j < data.size(); ++j)
          data(j) = Random();
        einspliner.set(i, data.data(), data.size());
      }
    }

    inline TinyVector<double, 3> test_all(const vector<pos_type>& Vpos, const vector<pos_type>& VGLpos,
        const vector<pos_type>& VGHpos)
    {
  ...
  struct random_position_generator
  {
    ...
    random_position_generator(int n, uint_type seed) :
      myrand(seed)
    {
      Vpos.resize(n);
      VGLpos.resize(n);
