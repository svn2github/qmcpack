...
namespace qmcplusplus
  ...
  enum {LOGPSI=0,       /*!< log(fabs(psi)) instead of square of the many-body wavefunction \f$|\Psi|^2\f$ */
        ...
        BRANCHINGENERGY, /*!< branching energy, the sum of all the components */
  ...
  struct Walker
    {
      ...
      int Age;
      ///Age of this walker age is incremented every step after crossing a node
      int ReleasedNodeAge;
      ...
      ///default constructor
      inline Walker() : ID(0),ParentID(0), Generation(0),Age(0), ReleasedNodeAge(0),
      ...
      inline explicit Walker(int nptcl) : ID(0),ParentID(0), Generation(0),Age(0), ReleasedNodeAge(0),
          Weight(1.0e0),Multiplicity(1.0e0)
      {
        Properties.resize(1,NUMPROPERTIES);
        resize(nptcl);
...
//       PropertyHistory[index].pop_back();
      ...
      inline int byteSize()
      {
        ...
        return 2*sizeof(long)+3*sizeof(int)+ PHindex.size()*sizeof(int)
      ...
      inline Msg& putMessage(Msg& m)
      {
        ...
        m << ID << ParentID << Generation << Age << ReleasedNodeAge;
